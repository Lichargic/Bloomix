...
export function Checklist({ showCategories, date }: ChecklistProps) {
  ...
  function commitTaskText(id: string, text: string) {
    ...
    editTask.reset()
    editTask.mutate({ id, text: nextText }, {
      onSuccess: () => {
        setDraftTaskTextState(prev => {
          const next = { ...prev }
          delete next[id]
            ...
            <>
              <header className="events-section-head">
                <div className="head-row">
                  ...
                  <span className="head-count" aria-label={`${events.length} event${events.length === 1 ? '' : 's'} today`}>
                    {events.length}
                  </span>
                </div>
              ...
              </div>{/* end events-list-fade */}
            </>
          )}
        </div>{/* end .checklist-scroll-regions */}
        ...
        <div className="checklist-add-slot">
          {addPhase === 'habit' ? (
            ...
            <div className="add-type-picker" role="group" aria-label="Choose what to add">
              ...
              <button type="button" className="type-pick-btn" onClick={() => { setModalOpen(true); setAddPhase('idle') }}>
                <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true" width="16" height="16">
              ...
              <button type="button" className="type-pick-cancel" onClick={() => setAddPhase('idle')} aria-label="Cancel">
            ...
            <button type="button" className="add-task-btn" disabled={createTask.isPending} onClick={() => setAddPhase('picking')}>
              <span className="add-icon" aria-hidden="true">+</span>
              <span>Add</span>
            </button>
          )}
        </div>{/* end .checklist-add-slot */}

      </div>{/* end .checklist-lists */}
