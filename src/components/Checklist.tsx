...
export function Checklist({ showCategories, date }: ChecklistProps) {
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) {
      if (e.key === 'Escape') { setAddPhase('idle'); setDraft('') }
    }
  ...
  const exactCategoryMatch = categoryOptions.some(category =>
  ...
  function commitTaskText(id: string, text: string) {
    ...
    if (timer) { window.clearTimeout(timer); delete editTimers.current[id] }
    const current = tasks.find(t => t.id === id)
    if (!current || text === current.text) return
    editTask.reset()
    editTask.mutate({ id, text }, {
      onSuccess: () => {
        setDraftTaskTextState(prev => { const next = { ...prev }; delete next[id]; return next })
      },
    })
  }
  ...
  const mutationBusy  = createTask.isPending || toggleTask.isPending || deleteTask.isPending
    ...
    <section className="checklist" data-screen-label="Checklist" aria-label="Today's checklist">
                  ...
                  {activeCategoryOptions.map(category => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => { setFilter(category); setMoreOpen(false) }}
                      >
                        <span className="tag-dot" style={{ background: cat.color }} aria-hidden="true" />
                        <span>{cat.label}</span>
          ...
          return (
            <li key={t.id} className={`task ${isDone ? 'is-done' : ''}`}>
              <button
                ...
                onClick={() => { toggleTask.reset(); toggleTask.mutate({ id: t.id, complete: !isDone }) }}
                aria-label={isDone ? `Mark "${t.text}" as not done` : `Mark "${t.text}" as done`}
                aria-pressed={isDone}
              >
                {isDone && (
                ...
                <input
                  ...
                  onKeyDown={e => {
                    ...
                    if (e.key === 'Escape') {
                      ...
                      if (timer) { window.clearTimeout(timer); delete editTimers.current[t.id] }
                      setDraftTaskTextState(prev => { const next = { ...prev }; delete next[t.id]; return next })
                    }
                  }}
                />
                {showCategories && (
              ...
              <button
                ...
                disabled={mutationBusy}
                onClick={() => { deleteTask.reset(); deleteTask.mutate(t.id) }}
      ...
      <div className="events-section">
        <div className="events-section-head">
          <span className="events-section-title">Events &amp; deadlines</span>
          <span className="events-section-sub">One-time, tied to a date</span>
        </div>
        <div className="deadlines events-deadlines">
          {events.length === 0 && (
            <p className="events-empty">Nothing scheduled today.</p>
          )}
          {events.map(t => {
      ...
      {addPhase === 'habit' ? (
        <div className="composer-card">
          <input
            ...
            aria-label="New habit description"
          />
          <div className="composer-category-field">
            <label className="composer-category-label" htmlFor="habit-category">Category</label>
            <div className="composer-combobox" ref={categoryComboboxRef}>
              <input
                ...
                onChange={e => { setCategoryQuery(e.target.value); setCategoryOpen(true) }}
                onKeyDown={e => {
                  if (e.key === 'Escape') {
                    setCategoryOpen(false)
                    setCategoryQuery(draftCat ? getCategoryConfig(draftCat).label : '')
                  ...
                  if (e.key === 'Enter' && categoryOpen) {
                    e.preventDefault()
                    if (matchingCategoryOptions[0]) selectDraftCategory(matchingCategoryOptions[0])
                    else createDraftCategory()
                  }
                }}
              />
              {categoryOpen && (
                <div id="habit-category-options" className="composer-category-menu open-up" role="listbox" aria-label="Category options">
                  {matchingCategoryOptions.map(category => {
                    ...
                    return (
                      ...
                      >
                        <span className="composer-category-icon" style={{ '--cat': cat.color } as React.CSSProperties} aria-hidden="true">
                          {cat.icon}
                        </span>
                        <span>{cat.label}</span>
                      </button>
          ...
          <div className="composer-actions">
            <button
              ...
              onClick={() => { setAddPhase('idle'); setDraft('') }}
            >
              Cancel
            </button>
            <button className="composer-submit" disabled={!draft.trim() || !draftCat || createTask.isPending} onClick={add}>
        ...
        <div className="add-type-picker" role="group" aria-label="Choose what to add">
          <button className="type-pick-btn" onClick={startHabitComposer}>
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true" width="16" height="16">
          ...
          <button className="type-pick-btn" onClick={() => { setModalOpen(true); setAddPhase('idle') }}>
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true" width="16" height="16">
          ...
          <button className="type-pick-cancel" onClick={() => setAddPhase('idle')} aria-label="Cancel">
        ...
        <button className="add-task-btn" disabled={createTask.isPending} onClick={() => setAddPhase('picking')}>
