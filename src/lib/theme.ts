...
export const TONE: Record<Tone, ToneConfig> = {
  soft: {
    greet: (name, pct) =>
      pct === 0
        ? `Hi ${name}. Pick one small thing to start.`
        : pct < 50
          ? `Nice start, ${name}. Your tree is stretching.`
          : pct < 100
            ? `Look at you, ${name}. The leaves are showing up.`
  ...
  whimsy: {
    greet: (name, pct) =>
      pct === 0
        ? `Yawn... ${name}, your tree just woke up.`
        : pct < 50
          ? `${name}! A leaf grew. (It saw you.)`
          : pct < 100
            ? `${name}, a bee just visited. Cool, right?`
            : `Full bloom! Your tree did a tiny dance.`,
    miss: 'Tree took a nap. It dreamed about you.',
    deadline: (n) =>
      n === 0 ? 'The week is wide open.' : `${n} tiny dragon${n > 1 ? 's' : ''} on the horizon.`,
  },
  matter: {
...
export function getTreeGreeting(tone: Tone, name: string, todayCare: number, treeStage: number) {
  ...
  if (stage < 3) {
    return pct === 0
      ? `The sprout is resting. One small task can help.`
      : `${name}, the sprout is settling in.`
  }

  if (stage < 6) {
    return pct === 0
      ? `The tree is steady. It can wait for care.`
      : `${name}, today's care moved the tree along.`
  }
