...
export const GARDEN_ARCHIVES_KEY = (userId: string) => ["garden_archives", userId] as const;

export const GARDEN_ACTIVITY_BASE_KEY = (userId: string) => ["garden_activity_stats", userId] as const;
	...
	return useQuery<GardenArchiveRecord[]>({
		...
		queryFn: async () => {
			...
			const { data, error } = await supabase
				...
				.select("id, season, tree_shape, days, blooms, tasks, archived_at, cycle_started_on, cycle_ended_on")
				.eq("user_id", user.id)
...
export function useGardenActivityStats(sinceDate: string | null) {
	...
	return useQuery<GardenActivityStats>({
		...
		queryFn: async () => {
			...
			const { data, error } = sinceDate ? await baseQuery.gt("completed_on", sinceDate) : await baseQuery;

			if (error) throw error;

			const completedDays = new Set((data ?? []).map((row) => row.completed_on).filter(Boolean));
