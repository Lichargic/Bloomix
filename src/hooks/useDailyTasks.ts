...
function useToday() {
	...
	const fallbackToday = useMemo(() => currentDailyDay(profile?.timezone ?? "UTC"), [profile?.timezone]);
	const { user } = useAuth();
...
export function normalizeDailyTasksForToday(tasks: DailyTask[], today: string): DailyTask[] {
	return tasks.map((task) => (task.completed_at === today ? task : { ...task, completed_at: null }));
}
...
export function useTendedDays() {
	...
	return useQuery<number>({
		...
		queryFn: async () => {
			const { data, error } = await supabase.rpc("count_tended_days");
			if (error) throw error;
...
export function useEditDailyTask() {
	...
	return useMutation({
		mutationFn: async ({ id, text }: { id: string; text: string }) => {
			if (!user) throw new Error("Not authenticated");
			const { error } = await supabase.from("daily_tasks").update({ text }).eq("id", id).eq("user_id", user.id);
			if (error) throw error;
		},
