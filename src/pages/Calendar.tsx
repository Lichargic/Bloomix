...
export function Calendar() {
	...
	useEffect(() => {
		...
		const defaultDay = isMonthParam(dateParam) ? `${month}-01` : today;
	...
	function dateInTargetMonth(targetYear: number, targetMonth0: number) {
		const sourceDay = selectedDay ? Number(selectedDay.slice(8, 10)) : 1;
	...
	return (
		<div className="app-shell">
				...
				<div className="cal-grid-wrap">
					...
					<div className="calendar-shell month-shell">
						...
						<div
							...
							role="grid"
							aria-label={`${monthName} ${y}`}>
							{cells.map((c, i) => {
								if (c.muted) {
									return (
										<span key={i} className="day muted" aria-hidden="true">
											{c.day}
										</span>
									);
								}
						...
						<div className="legend" aria-label="Legend">
							<span>
								<span className="leg-dot bloom" /> bloom
							</span>
							<span>
								<span className="leg-dot deadline" /> event
							</span>
						</div>
					</div>
					...
					<div className="calendar-shell day-detail" aria-label="Day detail">
								...
								{selectedDay && (
									<>
										...
										{selectedScheduled.length > 0 && (
											<ul
												className="task-list calendar-event-list"
												role="list"
												aria-label="Events and deadlines for selected day">
												{selectedScheduled.map((t) => {
													const cat = getCategoryConfig(t.category as Category);
													const isDone = t.completed_at !== null;
													const timeStr = formatCalendarEventTime(
														t.due_at,
														t.all_day,
														timezone,
													);
													return (
														<li
															key={t.id}
															className={`task calendar-event ${isDone ? "is-done" : ""}`}>
															<button
																className="task-check"
																onClick={() =>
																	toggleScheduled.mutate({
																		id: t.id,
																		complete: !isDone,
																	})
																}
						...
						{/* end day-detail-scroll-wrap */}
						{selectedDay && !dayEmptyState && (
							<button className="add-task-btn day-detail-add" onClick={() => setModalOpen(true)}>
								<span className="add-icon" aria-hidden="true">
									+
								</span>
						...
						<div className="upcoming-rail" ref={setRailRef}>
							{upcomingTasks.length === 0 && (
								<div className="upcoming-empty">
									<span className="calendar-empty-icon" aria-hidden="true">
										{CALENDAR_EMPTY_STATES.quietMonth.icon}
									</span>
									<div>
			...
			{modalOpen && selectedDay && <ScheduledTaskModal defaultDate={selectedDay} onClose={closeModal} />}
		</div>
	);
}
